...
func (c *CFG) PrintSubCFGTopology(firstPc uint) string {
	// Collect all nodes and edges
	nodes := make(map[*MIRBasicBlock]bool)
	edges := [][2]*MIRBasicBlock{}

	initialBBs := make([]*MIRBasicBlock, 0)
	initialBBs = append(initialBBs, c.pcToBlock[firstPc])
	for _, bb := range c.pcToVariants[firstPc] {
		initialBBs = append(initialBBs, bb)
	}
